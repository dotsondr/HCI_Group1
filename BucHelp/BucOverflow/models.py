...
class Comment(models.Model):
    ...
    def timedifference(self):
        ...
        t1=math.floor(timediff.total_seconds())

        t1=math.floor(t1/60) #minutes
        if((t1) > 1):   #more than a minute
            self.timeElapsed=str(t1)+" minutes ago" 
            t1=math.floor(t1/60) #hours
            if((t1) >= 1):   #an hour
                self.timeElapsed=str(t1)+" hours ago"

        elif(t1 == 1):  #one minute
            self.timeElapsed=str(t1)+" minute ago"
        else:           #less than a minute
            self.timeElapsed="just now"
